...
- `views[]` entry, `type: "gallery"` — `coverField?` (a url field rendered as the card cover; missing/broken → initials placeholder; default: the first url field) · `titleField?` (default: the primary field) · `metaFields?` (≤3 field keys under the title; select values as colored chips; default: the first 2 non-primary fields) · `cardSize?: "s" | "m" | "l"` (column width). Cards open the record peek; rendering is windowed for large objects.
- `views[]` entry, `type: "form"` — `fields?` (field keys to render, in order; default: every form-editable field — `json` and many-relations excluded) · `requiredOverrides?` (`{key: true|false}` over the primary-only default) · `submitLabel?` · `successMode?: "another" | "view"`. Submits through the object's create path; validation mirrors the server's config-implied rules and server messages map back onto their field.
- `views[]` entry, `type: "map"` — `latField`/`lngField` (the coordinate `number` fields; omitted → inferred from fields named `lat`/`latitude` and `lng`/`lon`/`long`/`longitude`, key first then label) · `titleField?` (pin/popup title; default: the primary) · `colorField?` (a `select` field whose option palette tints pins and cluster points). Records failing coordinate validity (non-number, out of range; 0 is valid) are counted in the "without location" chip, never dropped; past 25 located records rendering switches from per-record pins to GL clustering. Recipe: docs/RECIPES.md "Give an object a map view".
