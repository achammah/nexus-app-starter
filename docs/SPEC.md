...
| 15 | “keep improving the library so something like the ATS can be recreated easily” — config IS the app (entities+data+demo via sampleRows/CONFIG_PATH), relation pickers + related lists + select filters | journey-green | journeys: blocks-coverage-litmus (fixture: journeys/fixtures/coverage.config.json — see row 17) · relations-deep · select-filters; .playwright-mcp/ats-{board,record}.png | user 2026-07-19 |
...
| 17 | “the recreate-ATS is a TEST of the building blocks — I don’t want a config for an ATS” — the config is now a journey FIXTURE (`journeys/fixtures/coverage.config.json`), the journey is the blocks-coverage litmus, all shipped-ATS framing swept | journey-green | journeys: blocks-coverage-litmus; README litmus section | user correction 2026-07-19 |
