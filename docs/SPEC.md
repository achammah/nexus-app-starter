...
| 15 | “keep improving the library so something like the ATS can be recreated easily” — config IS the app (entities+data+demo via sampleRows/CONFIG_PATH), relation pickers + related lists + select filters; examples/ats.config.json boots a working ATS | journey-green | journeys: ats-by-config · relations-deep · select-filters; .playwright-mcp/ats-{board,record}.png | user 2026-07-19 |
