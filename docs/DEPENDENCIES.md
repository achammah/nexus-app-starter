...
| three | ^0.182.0 → 0.182.0 | MIT | the 3D viewer block's WebGL engine (`blocks/viewer3d`), with OrbitControls + GLTFLoader + RoomEnvironment from its examples tree. A STATIC import — absent, it breaks the BUILD | not measured | lazy view chunk (`LazyViewer3DSurface`) |
| exceljs | ^4.4.0 → 4.4.0 | MIT | the workbook block's XLSX + CSV round-trip (Univer's OSS engine ships no client-side .xlsx exchange) | not measured | dynamic `import()` inside `xlsx-io` — loads only on an actual import/export |
| docx | ^9.7.1 → 9.7.1 | MIT | `.docx` EXPORT from the document block's block model (`editor-io`) | not measured | dynamic `import()` at export time |
| mammoth | ^1.12.0 → 1.12.0 | BSD-2-Clause | `.docx` IMPORT into the document block's block model (`editor-io`) | not measured | dynamic `import()` at import time |
| react-is | ^19.2.8 → 19.2.8 | MIT | recharts' peer, declared directly (see the peer note below) | not measured | with recharts |
| rxjs | ^7.8.2 → 7.8.2 | Apache-2.0 | @univerjs' reactive core, declared directly | not measured | with the workbook chunk |
| lodash | ^4.17.21 → 4.18.1 | MIT | glide-data-grid / @univerjs peer, declared directly | not measured | with its consumers |
| marked | ^18.0.7 → 18.0.7 | MIT | glide-data-grid's markdown cell kind (unused by the Sheet view; tree-shaken) | not measured | with the grid chunk |
| react-responsive-carousel | ^3.2.23 → 3.2.23 | MIT | glide-data-grid's image cell kind (unused by the Sheet view; tree-shaken) | not measured | with the grid chunk |
| pdfjs-dist | 4.10.38 (pinned) → 4.10.38 | Apache-2.0 | renders PDF pages to canvas in the e-signature surface (`blocks/esign`). Ships an ES-module WORKER — see the `.mjs` MIME requirement in `docs/CONSTRAINTS.md` | not measured | lazy chunk (`LazyESignSurface`) |
| pdf-lib | 1.17.1 (pinned) → 1.17.1 | MIT | flattens placed fields + signatures into the signed PDF (`blocks/esign`) | not measured | lazy chunk (`LazyESignSurface`) |
...
**Why the peers are declared directly:** `react-is`, `rxjs`, `lodash`, `marked` and `react-responsive-carousel` are peers of recharts / glide-data-grid / @univerjs. They are listed as ordinary dependencies because `--legacy-peer-deps` turns peer resolution off, which would drop them from a clean clone's tree.

**Install (React 19):** `@glideapps/glide-data-grid@6.0.3` (latest) pins its peer to React ≤18 but renders correctly on 19, so npm's default resolution refuses the tree: every install needs `--force` (or `--legacy-peer-deps`), `npm ci` included — a plain `npm ci` fails with `ERESOLVE`. Both flags produce a complete tree because the packages that once arrived only as auto-installed peers of glide/recharts/@univerjs (`react-is`, `rxjs`, `lodash`, `marked`, `react-responsive-carousel`) are declared dependencies here. Leaving a needed package to peer resolution is the trap: `--legacy-peer-deps` disables peer resolution outright, so it vanishes from a clean clone's tree while remaining present for anyone who installed earlier.
...
| with gallery + form depth (group/sort/card fields; sections) | 1,314.66 kB | 389.50 kB | 159.51 kB |
...
| GalleryView (masonry, grouped sections + sort + card fields) | 10.03 kB | 4.06 kB | — |
| FormView (config-driven intake, sections + conditional required) | 5.97 kB | 2.40 kB | — |
...
Budget rule: the eager bundle must not grow more than 2% over the previous baseline without an explicit maintainer go (the registry landed at +0.51% min / +0.56% gzip; the Sheet view added +0.38% min / +0.45% gzip eager; the flow definition adds +0.41% min / +0.41% gzip over the Sheet-merged baseline; the field registry + whiteboard adds +0.73% min / +0.95% gzip over the Sheet+flow baseline — the field registry, the whiteboard definition + thumbnail shell, and the gallery's inline demo scene; excalidraw itself is fully lazy; the calendar view adds +0.43% gzip over the whiteboard baseline; the built-in draft editors + gallery/form views leave the eager bundle at 380.33 kB gzip — at or under the calendar baseline (the per-type Draft editors the create dialog and record page share are the only eager addition; both new views are fully lazy; vite keeps excalidraw's chunk fully lazy on this graph); the gallery + form depth pass leaves the eager bundle at 389.50 kB gzip — at or under the map baseline, the group-by/sort toolbar controls and the shared `views/sort.ts` util the only eager additions, both views staying lazy). New HEAVY view types register a `React.lazy` component (the registry host wraps rendering in Suspense), which code-splits them out of the eager chunk automatically; a lazy view chunk stays at or under ~250 KB gzip (the Sheet chunk sits at 104.19 KB gzip after the selection-contrast and native-integration polish added +0.64 KB gzip to the lazy chunk with the eager bundle unchanged, FlowView at 70.69 KB gzip, CalendarView at 77.88 KB gzip). The natural first split candidates in the existing set are recharts, react-day-picker and date-fns if the eager chunk needs to shrink.
