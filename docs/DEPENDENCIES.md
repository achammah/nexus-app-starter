...
| three | ^0.182.0 → 0.182.0 | MIT | the 3D viewer block's WebGL engine (`blocks/viewer3d`), with OrbitControls + GLTFLoader + RoomEnvironment from its examples tree. A STATIC import — absent, it breaks the BUILD | 38500 | lazy view chunk (`LazyViewer3DSurface`) |
| exceljs | ^4.4.0 → 4.4.0 | MIT | the workbook block's XLSX + CSV round-trip (Univer's OSS engine ships no client-side .xlsx exchange) | 22804 | dynamic `import()` inside `xlsx-io` — loads only on an actual import/export |
| docx | ^9.7.1 → 9.7.1 | MIT | `.docx` EXPORT from the document block's block model (`editor-io`) | 7384 | dynamic `import()` at export time |
| mammoth | ^1.12.0 → 1.12.0 | BSD-2-Clause | `.docx` IMPORT into the document block's block model (`editor-io`) | 2396 | dynamic `import()` at import time |
| react-is | ^19.2.8 → 19.2.8 | MIT | recharts' peer, declared directly (see the peer note below) | 32 | with recharts |
| rxjs | ^7.8.2 → 7.8.2 | Apache-2.0 | @univerjs' reactive core, declared directly | 11340 | with the workbook chunk |
