...
| polygon-clipping | ^0.15.7 → 0.15.7 | MIT | the whiteboard's boolean/shape ops (add/subtract/intersect/exclude/split) — the Martinez polygon-clipping algorithm behind Turf, over polygonised excalidraw shapes (`fields/whiteboard/geometry.ts`); transitive: splaytree 3.2.3 (MIT) | not measured | rides the whiteboard field's lazy path (pure geometry, no React, node-testable) |
